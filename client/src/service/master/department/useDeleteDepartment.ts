...
type TUseDeleteDepartmentProps = {
  onSuccess?: (data: null) => void;
  onError?: (error: unknown) => void;
};

const useDeleteDepartment = (props?: TUseDeleteDepartmentProps) => {
  const useDeleteDepartmentFn = async (id: string) => {
    try {
  ...
  const mutation = useMutation({
    ...
    mutationFn: useDeleteDepartmentFn,
    onSuccess: (response) => {
